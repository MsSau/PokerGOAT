-- Adds retraction: EscalationConfigView's Coach Directives panel (moved
-- there from CoachBriefView) lets a coach retract a directive they issued.
-- Never deleted — this app's evidence trail is append-only throughout
-- (Session Contract substitutions, WGP amendments, escalation events all
-- follow the same pattern: mark superseded, never destroy the record) — a
-- retracted directive stays visible with its retraction timestamp, it just
-- stops counting as "a directive predates this occurrence" for escalation
-- purposes going forward. Nullable, NULL = still active (the common case
-- and every pre-existing row).
ALTER TABLE "public"."coach_directives"
  ADD COLUMN IF NOT EXISTS "retracted_at" timestamptz;

-- Body-only change to perform_end_session: adds `retracted_at IS NULL` to
-- v_prior_directive's WHERE clause, same reasoning as 20260724160000's
-- execution_action_id tightening. Everything else in the function is
