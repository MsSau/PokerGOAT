-- Closes the #3b gap flagged in both escalationEngine.ts's and
-- perform_end_session's own comments: coach_directives had no
-- execution_action_id column, so ANY directive counted as "referencing
-- this action" for every action's escalation "after coach directive"
-- checks (major_after_coach_directive / critical_after_coaching_or_
-- intervention) — an unrelated directive could trigger an unrelated
-- action's escalation jump.
--
-- Nullable at the schema level (existing rows, all predating this column,
-- have nothing to backfill), but the new CoachBriefView directive-creation
-- form requires picking one — a directive is now always about a specific
-- Execution Action. A directive with no execution_action_id (any existing
-- row) simply never matches any action's WHERE clause below, so it
-- naturally stops counting toward escalation for anything — no separate
-- "untargeted" branch needed.
ALTER TABLE "public"."coach_directives"
  ADD COLUMN IF NOT EXISTS "execution_action_id" "uuid" REFERENCES "public"."execution_actions"("id");

-- Body-only change to perform_end_session: tightens v_prior_directive's
-- WHERE clause to the specific action, same reasoning as
-- 20260724150000's severity-history fix. Everything else in the function
-- is unchanged from that migration.
...
BEGIN
    ...
    LOOP
        ...
        -- Tightened per this migration: coach_directives now carries
        -- execution_action_id, so a directive only counts toward the
        -- specific action it names — an untargeted (legacy) directive has
        -- execution_action_id IS NULL and so matches nothing here.
        SELECT EXISTS (
