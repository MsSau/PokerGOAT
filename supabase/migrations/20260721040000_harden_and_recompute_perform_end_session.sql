-- Rectifies audit finding C1: perform_end_session was SECURITY DEFINER,
-- had no explicit GRANT/REVOKE (so it likely inherited Postgres's default
-- PUBLIC-execute grant), never compared the session's owning player against
-- auth.uid() (unlike its sibling perform_start_session, which does exactly
-- that), had no guard against finalizing an already-FINALIZED session, and
-- — the core of the finding — simply inserted whatever
-- p_execution_medal/p_outcome_medal/p_final_pnl/p_outcome_brm_compliant/
-- p_dimension_ratings/p_verdict_classification/p_escalation_updates the
-- CLIENT sent, with no server-side recomputation at all. Any authenticated
-- caller could finalize any session (including another player's) with
-- fully fabricated results.
--
-- This migration:
--   1. Adds the auth.uid() ownership check (mirrors perform_start_session).
--   2. Adds an idempotency guard: refuses to re-finalize an already-
--      FINALIZED session, rather than silently producing a second
--      revision_number=1 verdict row.
--   3. Adds SET search_path (perform_start_session already has this;
--      perform_end_session never did).
--   4. Explicitly GRANTs EXECUTE to authenticated only (never PUBLIC/anon),
--      matching perform_start_session's grant.
--   5. Re-derives every deterministic value INSIDE this function from the
--      actual underlying rows, instead of trusting client parameters:
--        - Session P&L, ITM rate, and final-table flag: summed from
--          tournaments/tournament_entries after finalizing them (§6),
--          not from a client-supplied p_final_pnl.
--        - BRM compliance / outcome hard-gate: from tournament_entries
--          actually flagged NON_COMPLIANT this session, not a client bool.
--        - Outcome Medal: outcomeEngine.ts's Gold→Silver→Bronze→None
--          hierarchy (§8), ported verbatim.
--        - Escalation stage transitions: escalationEngine.ts's 8-condition,
--          highest-satisfied-stage-wins precedence rule (§12), ported
--          verbatim, walking every occurrence in this session in
--          chronological order and reading/writing escalation_tracks
--          in-place so repeat occurrences of the same action within one
--          session correctly stack (this also fixes a latent bug: the old
--          client-side computeEscalationUpdateForOccurrence queried
--          execution_action_occurrences BEFORE this session's freshly-
--          tagged mistakes were ever inserted, so two same-session repeats
--          of one action could never see each other as a "prior
--          occurrence in the recent window" — recomputing here, after all
--          of this session's occurrences already exist, closes that gap).
--        - Dimension Severity Score / Rating / Execution Medal:
--          executionEngine.ts's hard-gate-first, base-points-plus-
--          escalation-bonus formula and medal thresholds (§11), ported
--          verbatim.
--        - Verdict classification + headline: verdictEngine.ts's
--          classifyVerdict/buildHeadline (§13), ported verbatim.
--        - Coaching priorities snapshot: read fresh from coaching_priorities
--          (status = ACTIVE) rather than trusting a client-supplied array.
--      p_tournament_finishes (player-reported tournament results) and
--      p_mistake_tags (player-tagged mistakes) remain client-supplied,
--      since nobody but the player knows those facts — but every
--      tournament_id/tournament referenced by either is now checked to
--      actually belong to this session before use. p_verdict_evidence
--      (the prose bullet text, potentially AI-rewritten — see
--      verdictProse.ts) also remains client-supplied: it is display text
--      describing already-computed facts, not itself a scored/financial
--      value, so it stays outside this function's trust boundary the same
--      way it already did for the AI rewrite step.
--
-- The function's parameter list has changed (10 trust-sensitive params
-- removed), so — following the exact precedent of
-- 20260718000001_drop_old_perform_start_session_overload.sql — the old
-- 16-arg overload must be explicitly dropped, not left behind: Postgres
-- resolves functions by (name, arg types), so CREATE OR REPLACE with a
-- different signature would otherwise create a second overload alongside
-- the vulnerable original rather than replacing it.
DROP FUNCTION IF EXISTS "public"."perform_end_session"(
  "p_session_id" "uuid",
  "p_final_pnl" numeric,
  "p_outcome_medal" "text",
  "p_outcome_brm_compliant" boolean,
  "p_outcome_hard_gate_triggered" boolean,
  "p_execution_medal" "text",
  "p_execution_hard_gate_triggered" boolean,
  "p_dimension_ratings" "jsonb",
  "p_verdict_classification" "text",
  "p_verdict_headline" "text",
  "p_verdict_evidence" "jsonb",
  "p_tournament_finishes" "jsonb",
  "p_mistake_tags" "jsonb",
  "p_escalation_updates" "jsonb",
  "p_coaching_priorities_snapshot" "jsonb",
  "p_behavioral_snapshot_id" "uuid"
);
...
BEGIN
    ...
    -- as a "prior occurrence" in the escalation walk below (see the
    -- migration header comment on the latent bug this also fixes).
    -- tournament_id, when present, is checked against session_id — the
    -- same ownership gap p_tournament_finishes had above.
    ...
    LOOP
        ...
        v_prior_major_ever := (v_current_stage >= 3);
        v_prior_critical_ever := (v_current_stage >= 5);
        v_prior_critical_recent := (v_prior_critical_ever AND v_prior_occurrences_recent > 0);
        ...
        IF v_occ.base_severity = 'MINOR' THEN
            IF v_current_stage = 0 THEN
                ...
                v_satisfied := v_satisfied || 'first_minor_at_baseline';
            END IF;
            IF v_prior_occurrences_recent > 0 THEN
                v_candidates := v_candidates || 2;
                v_satisfied := v_satisfied || 'repeat_minor_recent_window';
            END IF;
            IF v_prior_minor_at_stage2 THEN
                v_candidates := v_candidates || 3;
                v_satisfied := v_satisfied || 'minor_persists_into_short_term';
            END IF;
        END IF;
        ...
        IF v_occ.base_severity = 'MAJOR' THEN
            IF NOT v_prior_major_ever THEN
                v_candidates := v_candidates || 3;
                v_satisfied := v_satisfied || 'first_ever_major';
            END IF;
            IF v_prior_occurrences_recent > 0 THEN
                v_candidates := v_candidates || 4;
                v_satisfied := v_satisfied || 'repeat_major_recent_window';
            END IF;
            IF v_major_after_directive THEN
                v_candidates := v_candidates || 5;
                v_satisfied := v_satisfied || 'major_after_coach_directive';
            END IF;
            IF v_major_after_intervention THEN
                v_candidates := v_candidates || 6;
                v_satisfied := v_satisfied || 'major_after_intervention';
            END IF;
        END IF;
        ...
        IF v_occ.base_severity = 'CRITICAL' THEN
            IF NOT v_prior_critical_ever THEN
                v_candidates := v_candidates || 5;
                v_satisfied := v_satisfied || 'first_ever_critical';
            END IF;
            IF v_prior_critical_recent THEN
                v_candidates := v_candidates || 6;
                v_satisfied := v_satisfied || 'repeat_critical_recent_window';
            END IF;
            IF v_critical_after_coaching THEN
                v_candidates := v_candidates || 7;
                v_satisfied := v_satisfied || 'critical_after_coaching_or_intervention';
            END IF;
            IF v_current_stage >= 7 THEN
                v_candidates := v_candidates || 8;
                v_satisfied := v_satisfied || 'further_critical_after_stage3';
            END IF;
        END IF;
    ...
    -- P&L sign first: hard gate is checked before positive/negative.
    ...
    ELSIF v_good_execution AND v_positive THEN
        v_verdict_classification := 'PROFESSIONAL_WIN';
    ...
    -- 9. Verdict + evidence + generation context.
    INSERT INTO verdicts (id, session_id, revision_number, is_current, classification, headline, created_at)
    VALUES (v_verdict_id, p_session_id, 1, true, v_verdict_classification::verdict_classification, v_verdict_headline, NOW());
...
ALTER FUNCTION "public"."perform_end_session"(
  "p_session_id" "uuid", "p_tournament_finishes" "jsonb", "p_mistake_tags" "jsonb",
  "p_verdict_evidence" "jsonb", "p_behavioral_snapshot_id" "uuid"
) OWNER TO "postgres";

-- Explicit grant, mirroring perform_start_session's — closes the other
-- half of the finding (no grant/revoke previously existed, so this
-- SECURITY DEFINER function likely still carried Postgres's default
-- PUBLIC-execute grant).
GRANT ALL ON FUNCTION "public"."perform_end_session"(
  "p_session_id" "uuid", "p_tournament_finishes" "jsonb", "p_mistake_tags" "jsonb",
  "p_verdict_evidence" "jsonb", "p_behavioral_snapshot_id" "uuid"
) TO "authenticated";
