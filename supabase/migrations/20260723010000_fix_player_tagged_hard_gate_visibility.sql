-- Bug fix: perform_end_session's PLAYER_TAGGED mistake-tag insert (§2.6,
-- SessionReview.tsx's tap-to-tag card grid) hardcoded
-- is_non_compliant = false, hard_gate_triggered = false on every row,
-- regardless of the tagged execution_action's actual is_hard_gate flag.
-- coachBrief.ts's Critical Alerts and coachRoster.ts's roster-flagging
-- query both filter on `is_non_compliant = true OR hard_gate_triggered =
-- true` — so a player could self-tag a hard-gate violation via that card
-- grid and it would never surface on either coach-facing screen. Execution
-- Profile scoring/medals were never affected by this (that logic joins
-- execution_actions.is_hard_gate directly, not this column), only
-- visibility was silently broken.
--
-- Fix: look up the tagged action's is_hard_gate once per tag and use it
-- for both columns — mirrors tournaments.ts's SYSTEM_DETECTED path, which
-- already sets hard_gate_triggered from the same action property (that
-- path also always sets is_non_compliant = true unconditionally, since by
-- construction its three action names are always contract/BRM violations;
-- player-tagged mistakes span all four Execution dimensions, most of which
-- aren't compliance issues, so is_non_compliant only follows is_hard_gate
-- here rather than being unconditionally true).
--
-- Body-only change, no signature change, so CREATE OR REPLACE is
-- sufficient — same reasoning as this repo's other body-only
...
BEGIN
    ...
    -- hard_gate_triggered below — see this migration's header comment.
