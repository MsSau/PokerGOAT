-- Widens perform_end_session's verdict classification to cover two more
-- tiers of §13's evidence hierarchy — see verdictEngine.ts's header comment
-- for the full tier-by-tier coverage rationale (this migration is the SQL
-- port of that same change):
--
--   - Repeat-offences (§12 "Critical Escalation Stage", stage >=5): any
--     occurrence this session reaching that stage is now treated as the
--     same severity tier as a hard gate for classification purposes
--     (v_any_critical_escalation, tracked in the existing escalation walk
--     loop — one new line, no new query).
--   - Preparation: a NONE Preparation Medal can now withhold the
--     PROFESSIONAL_WIN classification, downgrading it to MIXED_SESSION —
--     and only in that one branch; it can never make a losing/hard-gated
--     session classify worse than it otherwise would (NULL-safe: a session
--     whose preparation_records lookup returns nothing, e.g. a pre-
--     mandatory-preparation legacy session, simply never matches
--     v_prep_medal = 'NONE' and so never downgrades).
--
-- Active Performance Framework and Behavioral Profile trend are
-- deliberately NOT added here — see verdictEngine.ts's header comment for
-- why (Framework doesn't describe this session's own conduct; Behavioral
-- Profile trend has no SQL port of its rolling-window computation to draw
-- on here). Both are instead surfaced only as AI Reflection context
-- (server.ts's /api/verdict-reflection), computed client-side in
-- endSession.ts, entirely outside this function.
--
-- Body-only change (v_prep_medal is looked up via v_prep_record_id, already
-- resolved at step 0 for verdict_generation_context — no new param), so
-- CREATE OR REPLACE is sufficient, same reasoning as this repo's other
-- body-only perform_end_session migrations.
