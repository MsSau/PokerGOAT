-- perform_start_session's "max two sessions per poker day" gate was using a
-- rolling 24-hour lookback (`now() - interval '24 hours'`) rather than the
-- product's actual "Poker Day" concept (a configurable boundary time, PRD
-- §3/§4, poker_week_boundary_configs.poker_day_boundary_time — the same
-- boundary already used client-side in sessionContract.ts's
-- computeTodayBoundaries and shown on the Player Dashboard as "Current Poker
-- Day window"). A rolling window incorrectly blocks a session that's on a
-- different Poker Day but happens to fall within 24 raw hours of a session
-- from the day before, and mismatches whatever the Weekly Game Plan/BRM
-- capacity math thinks "today" is. Recompute the cap against the actual
-- Poker Day boundary, resolved via the contract's own weekly game plan ->
-- poker week -> boundary config chain, interpreted in the player's
-- profiles.timezone (a more reliable source server-side than the client's
-- browser-local-clock MVP simplification).
