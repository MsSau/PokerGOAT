-- DEV-ONLY, TEMPORARY: raises perform_start_session's "max two sessions per
-- poker day" cap so local development/testing isn't blocked by it while
-- iterating on session-related features. This is a relaxation, not a
-- removal of the product rule — the check and its Poker Day boundary math
-- (see 20260720063139_fix_poker_day_session_cap.sql) are untouched, only the
-- `>= 2` threshold changes to `>= 1000`.
--
-- TO REVERT: re-run 20260720063139_fix_poker_day_session_cap.sql's
-- CREATE OR REPLACE FUNCTION body verbatim (it's still in that file,
-- unchanged), or flip the `>= 1000` below back to `>= 2` in a new migration.
-- Do this before any real player-facing usage — the two-session cap is a
-- real PRD-specified guardrail, not dead code.
...
begin
  ...
  -- DEV RELAXATION: >= 2 raised to >= 1000 — see file header. Still
  -- enforced here (not just the UI), just effectively inert for now.
  if (
