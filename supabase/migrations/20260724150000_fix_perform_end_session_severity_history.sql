-- Mirrors escalationEngine.ts's computeEscalationUpdateForOccurrence fix:
-- "was there ever a prior MAJOR/CRITICAL occurrence for this track" was
-- being inferred from raw escalation stage position
-- (v_prior_major_ever := v_current_stage >= 3, v_prior_critical_ever :=
-- v_current_stage >= 5), which is wrong whenever a track reached that
-- stage via a DIFFERENT severity's path — e.g. MINOR can climb to stage 3
-- the same way a first-ever MAJOR does (minor_persists_into_short_term),
-- so a track sitting at stage 3 from a run of MINORs would have
-- incorrectly satisfied "first_ever_major" as false (i.e. NOT first-ever)
-- the moment a real MAJOR occurrence showed up, same bug for MAJOR-via-
-- directive reaching stage 5 vs. a genuine first CRITICAL.
--
-- Reconstructed instead from which condition string actually satisfied
-- each PAST transition (escalation_events.satisfied_conditions, persisted
-- by this same function every time it runs) — the real record of what
-- severity path fired, not a position on the ladder. Joined via
-- escalation_tracks(player_id, execution_action_id) rather than track_id,
-- since v_track_id isn't resolved until later in the loop.
--
-- v_session_severity_log is the SQL-side equivalent of escalationEngine.ts's
-- priorWalkEvents param: escalation_events rows for THIS session aren't
-- inserted until the end of each loop iteration, and even once inserted,
-- their created_at is NOW() (the transaction's start time, constant for
-- the whole function call) — chronologically AFTER any v_occ.occurred_at
-- from earlier in the same session, which would silently break the 7-day
-- recency window for a same-session repeat. The "ever" checks don't
-- strictly need this (same-transaction writes are visible to later
-- reads), but it's included for all three checks for symmetry with the TS
-- fix and so correctness doesn't depend on that visibility subtlety.
--
-- Body-only change (new DECLAREs + one section of the escalation walk),
-- same CREATE OR REPLACE reasoning as this repo's other body-only
-- perform_end_session migrations.
...
DECLARE
    ...
    -- True same-severity history (see this migration's header) — the
...
BEGIN
    ...
    LOOP
        ...
        -- history + this-session log), not stage position. See this
        -- migration's header for why the old (v_current_stage >= N) check
        -- was wrong.
        SELECT (
            EXISTS (
        ...
        -- this migration's header on why escalation_events alone isn't
