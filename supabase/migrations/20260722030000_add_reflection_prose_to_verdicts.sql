-- Adds persistence for player free text that was previously either never
-- read back (preparation_records.optional_note, session_contracts.
-- session_intention, tournaments.comments) or actively discarded
-- (endSession.ts accepted a `reflectionNote` param and silently dropped it;
-- PreGameRitual.tsx kept its three free-text answers in local state only),
-- plus a new column to store the immutable AI-generated "Reflection"
-- paragraph synthesized from that free text (see server.ts's new
-- POST /api/verdict-reflection route).
--
-- Three ALTER TABLEs:
--   sessions.reflection_note        — the end-of-session "Be specific"
--                                      reflection textarea (§2.7).
--   verdicts.reflection_prose       — the AI-generated closing paragraph,
--                                      written once at finalize time,
--                                      immutable thereafter like the rest
--                                      of the Verdict.
--   preparation_records.ritual_*    — the Pre-Game Ritual's three
--                                      free-text prompts (Intent, Identity
--                                      line, Process definition), colocated
--                                      with the existing
--                                      pre_game_ritual_completed boolean on
--                                      the same row.
--
-- Then perform_end_session gains two new trailing params, both
-- DEFAULT NULL, so CREATE OR REPLACE is sufficient (no DROP FUNCTION
-- needed — same reasoning as this repo's other additive-parameter
-- perform_end_session migrations).

ALTER TABLE "public"."sessions" ADD COLUMN "reflection_note" "text";
ALTER TABLE "public"."verdicts" ADD COLUMN "reflection_prose" "text";
ALTER TABLE "public"."preparation_records"
  ADD COLUMN "ritual_intent" "text",
  ADD COLUMN "ritual_identity_line" "text",
  ADD COLUMN "ritual_process_definition" "text";
