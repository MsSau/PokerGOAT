-- Fixes a bug introduced by 20260721040000_harden_and_recompute_perform_end_session.sql:
-- `v_satisfied := v_satisfied || '<literal>';` throws
-- `malformed array literal: "<literal>"` at runtime. Postgres's operator
-- resolution for `||` is ambiguous between the anyarray||anyarray and
-- anyarray||anyelement overloads when the right-hand side is an untyped
-- string literal; it picks the array||array overload and tries to parse
-- the plain string as an array constant (which must look like '{...}'),
-- failing immediately. This broke every session finalization that hit any
-- MINOR/MAJOR/CRITICAL escalation branch (i.e. any occurrence with prior
-- history), since v_satisfied is TEXT[] and every append used this pattern.
--
-- Fix: use array_append(), which is unambiguous, everywhere v_satisfied is
-- built up. No signature change, so CREATE OR REPLACE is sufficient (no
-- DROP FUNCTION needed, unlike the parameter-list changes in prior
...
BEGIN
    ...
    LOOP
        ...
        -- — see this migration's header comment for why the `||` form
